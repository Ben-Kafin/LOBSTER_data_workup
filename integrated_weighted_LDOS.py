...
class ldos_single_point:
    ...
    def calculate_single_point_ldos(self, position, emin, emax, phi, V):
        ...
        for atom_idx, atom_pdos in enumerate(self.pdos):
            ...
            for orbital, spin_data in atom_pdos.items():
                ...
                # Initialize dictionary for this orbital if not present
                if orbital not in ldos[atom_idx]:
                    ldos[atom_idx][orbital] = {}
                
                for spin, dos_values in spin_data.items():
                    print(f"    Spin: {spin}")  # Debugging output
                    ...
                    # Tunneling weights (applied at each energy point)
                    ...
                    # New feature: Integrate the area under the pmodos curve between each energy point and the Fermi level.
                    # Since the energies are relative to the Fermi level, we assume E = 0 corresponds to the Fermi level.
                    f_idx = np.argmin(np.abs(energy_range - 0))
                    integrated_weights = np.zeros_like(energy_range)
                    for i in range(len(energy_range)):
                        if i < f_idx:
                            # for energies below the Fermi level, integrate from the current energy up to E_Fermi (index f_idx)
                            integrated_weights[i] = np.trapz(energy_filtered_dos[i:f_idx+1], energy_range[i:f_idx+1])
                        else:
                            # for energies above the Fermi level, integrate from E_Fermi (index f_idx) up to the current energy
                            integrated_weights[i] = np.trapz(energy_filtered_dos[f_idx:i+1], energy_range[f_idx:i+1])
                    print(f"    Integrated weights: {integrated_weights}")  # Debugging output
                    
                    # Combine the tunneling factor with the integrated weight
                    combined_weights = tunneling_weights * integrated_weights
                    print(f"    Combined weights: {combined_weights}")  # Debugging output
                    
                    # Weighted LDOS contribution: multiply pointwise DOS with the combined weight
                    ldos_contrib = energy_filtered_dos * combined_weights
                    print(f"    LDOS contribution (spin {spin}) with integrated weight: {ldos_contrib}")  # Debugging output
        
                    # Accumulate contributions for this spin channel
                    # (If the key doesn't exist yet, initialize it with the current contribution)
                    if spin in ldos[atom_idx][orbital]:
    ...
    def plot_ldos_curve(self, ldos, emin, emax):
        ...
        # Filter to include only curves with normalized area > 0.75 (or as needed)
...
if __name__ == "__main__":
    filepath = 'directory/'  # Directory containing output files
