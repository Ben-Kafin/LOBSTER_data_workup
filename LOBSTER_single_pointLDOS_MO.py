...
class ldos_single_point:
    ...
    def plot_ldos_curve(self, ldos, emin, emax):
        """
        Plots the total and orbital-resolved LDOS for each fragment across the energy window.
        The plotting routine integrates the orbital contributions, scales them, and displays
        distinct curves for each fragment's orbital.
        """
        energy_range = self.energies[self.estart:self.eend]
        ...
        for frag_idx, fragment_dos in ldos.items():
            for orbital, spin_data in fragment_dos.items():
                ...
                areas[(frag_idx, orbital)] = area
                total_ldos += orbital_ldos

        if not areas:
            print("No orbital contributions found in the LDOS data. "
                  "Please verify that your LCFO DOSCAR file and MO mappings contain valid pMODOS data.")
            return

        max_area = max(areas.values())
        normalized_areas = {key: area / max_area for key, area in areas.items()}
        filtered_areas = {key: area for key, area in normalized_areas.items() if area > 0.5}
        ...
        for (frag_idx, orbital), area in filtered_areas.items():
            orbital_ldos = np.zeros_like(energy_range)
            for spin, ldos_values in ldos[frag_idx][orbital].items():
                orbital_ldos += ldos_values
            plt.plot(energy_range, orbital_ldos,
                     label=f"Fragment {frag_idx+1}, Orbital {orbital} (Norm. Area: {normalized_areas[(frag_idx, orbital)]:.2f})",
                     color=atom_colors[frag_idx], alpha=0.7)
...
if __name__ == "__main__":
    filepath = 'C:/Users/nazin_lab/Documents/VASP_files/NHCs/iPr/lone_4layers/freegold1/freegold2/tall50_on'
    spatial_position = np.array([5.00732, 8.67293, 23])
    emin, emax = -2.0, 2.0  # Energy window (eV)
    phi = 5.0885            # Workfunction (eV)
    V = 2.0                 # Applied voltage (eV)
